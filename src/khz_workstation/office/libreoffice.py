...
class LibreOfficeEngine(IOfficeEngine):
    ...
    def info(self) -> OfficeEngineInfo:
        ...
        if self.executable:
            try:
                cp = subprocess.run([str(self.executable), "--version"], capture_output=True, text=True, timeout=10, encoding="utf-8", errors="replace")
                version = (cp.stdout or cp.stderr).strip() or None
            except Exception:
                version = None
        return OfficeEngineInfo("LibreOffice", str(self.executable) if self.executable else None, version, bool(self.executable), "out-of-process local desktop editor; deterministic headless conversion")
    ...
    def open_for_edit(self, path: Path) -> int | None:
        ...
        proc = subprocess.Popen([str(self.executable), "--norestore", "--nofirststartwizard", str(path)])
        return proc.pid
    ...
    def convert_to_pdf(self, path: Path, output_dir: Path) -> Path:
        ...
        cp = subprocess.run([str(self.executable), "--headless", "--norestore", "--nofirststartwizard", "--convert-to", "pdf", "--outdir", str(output_dir), str(path)], capture_output=True, text=True, timeout=120, encoding="utf-8", errors="replace")
        if cp.returncode != 0:
